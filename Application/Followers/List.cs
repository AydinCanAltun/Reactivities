...
namespace Application.Followers
{
    public class List
    {
        ...
        public class Handler : IRequestHandler<Query, List<Profile>>
        {
            ...
            {
                var quearyable = _context.Followings.AsQueryable();
                ...
                switch(request.Predicate)
                {
                    case "followers":
                    {
                        userFollowings = await quearyable.Where(x => x.Target.UserName == request.Username).ToListAsync();
                    ...
                    case "following":
                    {
                        userFollowings = await quearyable.Where(x => x.Observer.UserName == request.Username).ToListAsync();
